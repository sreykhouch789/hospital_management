...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
            <i class="fa-solid fa-bed text-emerald-400"></i> {{ __('app.rooms_title') }}
        </h1>
        <p class="text-xs text-slate-400">{{ __('app.rooms_subtitle') }}</p>
    </div>
        ...
        <div class="glass-panel p-5 rounded-2xl border border-slate-800 flex flex-col justify-between hover:border-emerald-500/40 transition-all relative">
            <div>
                ...
                <div class="mt-3 space-y-1.5 text-xs">
                    <div class="text-slate-400">{{ __('app.room_type') }}: <span class="text-slate-200 font-semibold">{{ $room->type }}</span></div>
                    <div class="text-slate-400">{{ __('app.daily_rate') }}: <span class="text-emerald-400 font-mono font-bold">${{ number_format($room->daily_rate, 2) }}</span> / day</div>
            ...
            <div class="mt-4 pt-3 border-t border-slate-800 flex items-center justify-between">
                ...
                @elseif($room->status === 'Occupied')
                    <form action="{{ route('rooms.discharge', $room->id) }}" method="POST" class="w-full">
                        @csrf
                        @method('PATCH')
                        <button type="submit" onclick="return confirm('{{ __('app.discharge_confirm') }}')" class="w-full bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 text-xs font-semibold py-1.5 rounded-lg border border-rose-500/30">
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 class="text-base font-bold text-white flex items-center gap-2">
                <i class="fa-solid fa-plus text-emerald-400"></i> {{ __('app.add_room') }}
            </h3>
        ...
        <form action="{{ route('rooms.store') }}" method="POST" class="space-y-4 text-xs">
            @csrf
            <div>
                <label class="block text-slate-300 font-medium mb-1">{{ __('app.room_no') }}</label>
                <input type="text" name="room_number" placeholder="{{ __('app.room_number_placeholder') }}" required class="w-full bg-slate-800 border border-slate-700 text-white rounded-lg px-3 py-2 focus:outline-none focus:border-emerald-500">
            </div>

            <div>
                <label class="block text-slate-300 font-medium mb-1">{{ __('app.room_type') }}</label>
            ...
            <div>
                <label class="block text-slate-300 font-medium mb-1">{{ __('app.daily_rate') }} ($)</label>
            ...
            <div class="pt-3 flex justify-end gap-3 border-t border-slate-800">
                <button type="button" onclick="document.getElementById('addRoomModal').classList.add('hidden')" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg">{{ __('app.cancel') }}</button>
                <button type="submit" class="px-5 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-lg shadow-md shadow-emerald-500/20">{{ __('app.create_room') }}</button>
            </div>
        </form>
    </div>
</div>
...
<div id="assignModal" class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center hidden p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 class="text-base font-bold text-white flex items-center gap-2">
                <i class="fa-solid fa-bed text-emerald-400"></i> {{ __('app.assign_patient_modal') }} <span id="roomNumberText" class="font-mono text-emerald-400"></span>
            </h3>
        ...
        <form id="assignForm" method="POST" class="space-y-4 text-xs">
            ...
            <div class="pt-3 flex justify-end gap-3 border-t border-slate-800">
                <button type="button" onclick="document.getElementById('assignModal').classList.add('hidden')" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg">{{ __('app.cancel') }}</button>
                <button type="submit" class="px-5 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-lg shadow-md shadow-emerald-500/20">{{ __('app.assign') }}</button>
            </div>
        </form>
    </div>
</div>
