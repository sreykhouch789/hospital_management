...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
            <i class="fa-solid fa-user-doctor text-emerald-400"></i> Doctors Directory
        </h1>
        <p class="text-xs text-slate-400">Manage medical specialists, schedules, and consultation fees.</p>
    </div>
...
<form method="GET" action="{{ route('doctors.index') }}" class="glass-panel p-4 rounded-xl border border-slate-800 flex flex-col sm:flex-row gap-4">
    <div class="flex-1 relative">
        ...
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search doctor name or specialization..." class="w-full bg-slate-900 border border-slate-700 text-xs text-slate-200 rounded-lg pl-8 pr-4 py-2 focus:outline-none focus:border-emerald-500">
    </div>
        ...
        <div class="glass-panel p-5 rounded-2xl border border-slate-800 flex flex-col justify-between hover:border-emerald-500/40 transition-all group relative">
            <div>
                <div class="flex items-start justify-between">
                    ...
                    <form action="{{ route('doctors.destroy', $doctor->id) }}" method="POST" onsubmit="return confirm('Remove doctor?')">
                        @csrf
                        @method('DELETE')
                ...
                <div class="mt-4 pt-4 border-t border-slate-800 space-y-2 text-xs text-slate-300">
                    <div class="flex items-center justify-between">
                        <span class="text-slate-400"><i class="fa-solid fa-building text-emerald-400 w-4"></i> Dept:</span>
                        <span class="font-medium text-white">{{ $doctor->department->name ?? 'N/A' }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-slate-400"><i class="fa-solid fa-phone text-emerald-400 w-4"></i> Phone:</span>
                    ...
                    <div class="flex items-center justify-between">
                        <span class="text-slate-400"><i class="fa-solid fa-dollar-sign text-emerald-400 w-4"></i> Fee:</span>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 class="text-base font-bold text-white flex items-center gap-2">
                <i class="fa-solid fa-user-plus text-emerald-400"></i> Register New Doctor
            </h3>
        ...
        <form action="{{ route('doctors.store') }}" method="POST" class="space-y-4 text-xs">
            @csrf
            <div>
                <label class="block text-slate-300 font-medium mb-1">Doctor Full Name</label>
                <input type="text" name="name" placeholder="Dr. John Doe" required class="w-full bg-slate-800 border border-slate-700 text-white rounded-lg px-3 py-2 focus:outline-none focus:border-emerald-500">
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    <label class="block text-slate-300 font-medium mb-1">Specialization</label>
                    <input type="text" name="specialization" placeholder="Cardiologist" required class="w-full bg-slate-800 border border-slate-700 text-white rounded-lg px-3 py-2 focus:outline-none focus:border-emerald-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="email" name="email" placeholder="doctor@hospital.com" required class="w-full bg-slate-800 border border-slate-700 text-white rounded-lg px-3 py-2 focus:outline-none focus:border-emerald-500">
                </div>
                <div>
                    <label class="block text-slate-300 font-medium mb-1">Phone</label>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-slate-300 font-medium mb-1">Fee ($)</label>
                ...
                <div>
                    <label class="block text-slate-300 font-medium mb-1">Status</label>
            ...
            <div class="pt-3 flex justify-end gap-3 border-t border-slate-800">
                <button type="button" onclick="document.getElementById('addDoctorModal').classList.add('hidden')" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg">Cancel</button>
                <button type="submit" class="px-5 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-lg shadow-md shadow-emerald-500/20">Save Doctor</button>
            </div>
        </form>
    </div>
</div>
