...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
            <i class="fa-solid fa-file-invoice-dollar text-emerald-400"></i> {{ __('app.bills_title') }}
        </h1>
        <p class="text-xs text-slate-400">{{ __('app.bills_subtitle') }}</p>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left text-xs">
            ...
            <tbody class="divide-y divide-slate-800/60">
                @forelse($bills as $bill)
                    <tr class="hover:bg-slate-800/40 transition-colors">
                        ...
                        <td class="py-3.5 px-4">
                            <div class="font-bold text-white">{{ $bill->patient->name ?? __('app.na') }}</div>
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center py-8 text-slate-500">{{ __('app.no_bills') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 class="text-base font-bold text-white flex items-center gap-2">
                <i class="fa-solid fa-receipt text-emerald-400"></i> {{ __('app.add_bill') }}
            </h3>
        ...
        <form action="{{ route('bills.store') }}" method="POST" class="space-y-4 text-xs">
            ...
            <div>
                <label class="block text-slate-300 font-medium mb-1">{{ __('app.select_appointment') }}</label>
            ...
            <div class="grid grid-cols-3 gap-3">
                ...
                <div>
                    <label class="block text-slate-300 font-medium mb-1">{{ __('app.room_charges') }} ($)</label>
                ...
                <div>
                    <label class="block text-slate-300 font-medium mb-1">{{ __('app.medication_fee') }} ($)</label>
            ...
            <div class="pt-3 flex justify-end gap-3 border-t border-slate-800">
                <button type="button" onclick="document.getElementById('addBillModal').classList.add('hidden')" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg">{{ __('app.cancel') }}</button>
                <button type="submit" class="px-5 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-lg shadow-md shadow-emerald-500/20">{{ __('app.create_invoice') }}</button>
            </div>
        </form>
    </div>
</div>
